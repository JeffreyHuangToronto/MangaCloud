...
const Search = () => {
    ...
    async function search(search_query) {
        ...
        await fetch(
            `https://mangacloud.azurewebsites.net/api/manga/database/search?search_query=${search_query}&source=MangaKakalot`,
            requestOptions
        ).then(async (response) => {
    ...
    return (
        <View style={styles.scroll}>
            ...
            <FlatList data={searchResults} renderItem={manga} keyExtractor={(manga) => manga._id} numColumns={3} />
        </View>
    );
};
